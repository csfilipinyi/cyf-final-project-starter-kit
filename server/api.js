    ...
    (err, result) => {
      if (result.rowCount > 0) {
        ...
        Connection.connect((err) => {
          ...
          Connection.query(
            "insert into graduates (first_name, surname, city, personal_bio,past_experience,github_id) values" +
              "($1,$2,$3,$4,$5,$6)",
            [
              newFirstName,
              newSurname,
...
router.get("/accounts/:name", (_, res, next) => {
  ...
  Connection.connect((err) => {
    ...
    Connection.query(
      ...
      (error, result) => {
        ...
        else res.status(400).send("this is not  a graduate");
      }
    );
  });
});
